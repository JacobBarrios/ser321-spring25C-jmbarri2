...
class WebServer {
  ...
  static {
    _images.put("streets", "https://iili.io/JV1pSV.jpg");
    _images.put("bread", "https://iili.io/Jj9MWG.jpg");
  }
  ...
  public byte[] createResponse(InputStream inStream) {
    ...
    try {
      ...
      if (request == null) {
        ...
        if (request.length() == 0) {
          ...
          try {
            ...
            if (json.contains("name")) {
              // Example: parse the JSON manually (this could be enhanced with a proper library)
              String[] items = json.split("\"name\":");
              for (int i = 1; i < items.length; i++) {
                String repoName = items[i].split(",")[0].replaceAll("[\"{}]", "").trim();
          ...
          if(str1 == null || str2 == null || str1.isEmpty() || str2.isEmpty()) {
            ...
            builder.append("Content-Type: text/html; charset=utf-8\n");
            builder.append("\n");
            builder.append("Error: Both 'str1' and 'str2' must be provided and non-empty.");
          }
          else {
            builder.append("HTTP/1.1 200 OK\n");
            builder.append("Content-Type: text/html; charset=utf-8\n");
        ...
        else if (request.contains("random?")) {
          Map<String, String> query_pairs = splitQuery(request.replace("random?", ""));
          
          String minStr = query_pairs.get("min");
          String maxStr = query_pairs.get("max");
          
          if(minStr == null || maxStr == null) {
            builder.append("HTTP/1.1 400 Bad Request\n");
            builder.append("Content-Type: text/html; charset=utf-8\n");
            builder.append("\n");
            builder.append("Error: Both 'min' and 'max' parameters are required.");
          }
          else {
            try {
              ...
              if(min > max) {
                ...
                builder.append("Content-Type: text/html; charset=utf-8\n");
                builder.append("\n");
                builder.append("Error: 'min' should be less than or equal to 'max'.");
              }
              else {
                ...
                builder.append("Content-Type: text/html; charset=utf-8\n");
                builder.append("\n");
                builder.append("Random number: " + randomValue);
              }
            }
              ...
              builder.append("Content-Type: text/html; charset=utf-8\n");
              builder.append("\n");
              builder.append("Error: 'min' and 'max' must be valid integers.");
            }
          }
        }
        else {
